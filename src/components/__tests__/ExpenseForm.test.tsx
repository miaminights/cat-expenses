...
describe('validateForm', () => {
  it('returns no errors for a fully valid form', () => {
    expect(
      validateForm({ name: 'Cat Toy', category: 'Accessory', amount: '9.99' }),
    ).toEqual({})
  })
  ...
  it('rejects names longer than 100 characters', () => {
    expect(
      validateForm({ name: 'a'.repeat(101), category: 'Food', amount: '10' }),
    ).toMatchObject({ name: 'Item name must be 100 characters or fewer.' })
  })
...
describe('ExpenseForm component', () => {
  ...
  it('trims leading/trailing whitespace from the name before submitting', async () => {
    ...
    expect(onSubmit).toHaveBeenCalledWith(
      expect.objectContaining({ name: 'Cat Toy' }),
    )
  })
