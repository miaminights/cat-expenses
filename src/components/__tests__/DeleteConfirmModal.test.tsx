...
describe('DeleteConfirmModal visibility', () => {
  it('renders nothing when isOpen is false', () => {
    render(
      <DeleteConfirmModal isOpen={false} onClose={vi.fn()} onConfirm={vi.fn()} expenses={[catFood]} />,
    );
  ...
  it('renders the dialog when isOpen is true', () => {
    render(
      <DeleteConfirmModal isOpen={true} onClose={vi.fn()} onConfirm={vi.fn()} expenses={[catFood]} />,
    );
...
describe('DeleteConfirmModal description', () => {
  it('uses singular "expense" when one expense is selected', () => {
    ...
    expect(
      screen.getByText('The following expense will be permanently deleted.'),
    ).toBeInTheDocument();
  });
  ...
  it('uses plural "expenses" when multiple expenses are selected', () => {
    render(
      <DeleteConfirmModal
        ...
        onClose={vi.fn()}
        onConfirm={vi.fn()}
        expenses={[catFood, catToy]}
      />,
    );
    expect(
      screen.getByText('The following expenses will be permanently deleted.'),
    ).toBeInTheDocument();
  });
});
  ...
  it('renders a row for each selected expense', () => {
    render(
      <DeleteConfirmModal
        ...
        onClose={vi.fn()}
        onConfirm={vi.fn()}
        expenses={[catFood, catToy, catBed]}
      />,
    );
...
describe('DeleteConfirmModal callbacks', () => {
  it('calls onClose when the Cancel button is clicked', async () => {
    const onClose = vi.fn();
    render(
      <DeleteConfirmModal isOpen={true} onClose={onClose} onConfirm={vi.fn()} expenses={[catFood]} />,
    );
  ...
  it('calls onConfirm when the Delete button is clicked', async () => {
    ...
    render(
      <DeleteConfirmModal isOpen={true} onClose={vi.fn()} onConfirm={onConfirm} expenses={[catFood]} />,
    );
