...
describe('AutoFocus', () => {
  it('renders the child element', () => {
    render(
      <AutoFocus>
        {(ref) => (
          <button ref={ref as React.Ref<HTMLButtonElement>}>Click me</button>
        )}
      </AutoFocus>,
    );
  ...
  it('focuses the element after mount', () => {
    render(
      <AutoFocus>
        {(ref) => (
          <button ref={ref as React.Ref<HTMLButtonElement>}>Focused</button>
        )}
      </AutoFocus>,
    );
  ...
  it('works with input elements', () => {
    render(
      <AutoFocus>
        {(ref) => (
          <input ref={ref as React.Ref<HTMLInputElement>} placeholder="Name" />
        )}
      </AutoFocus>,
    );
  ...
  it('does not throw when ref is not attached to any element', () => {
    expect(() =>
      render(<AutoFocus>{() => <span>No ref</span>}</AutoFocus>),
    ).not.toThrow();
  });
  ...
  it('only focuses the element that received the ref, not siblings', () => {
    render(
      <div>
        <AutoFocus>
          {(ref) => (
            <button ref={ref as React.Ref<HTMLButtonElement>}>Target</button>
          )}
        </AutoFocus>
        <button>Other</button>
      </div>,
