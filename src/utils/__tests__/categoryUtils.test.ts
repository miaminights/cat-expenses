...
const expense = (
  ...
  amount: number,
): Expense => ({ id, name: `Item ${id}`, category, amount })
...
describe('getTopCategories', () => {
  ...
  it('returns all categories that are tied for the highest total', () => {
    const expenses = [
      expense('1', 'Food', 100),
      expense('2', 'Accessory', 100),
      expense('3', 'Furniture', 50),
    ]
  ...
  it('returns all three categories when all totals are equal', () => {
    const expenses = [
      expense('1', 'Food', 10),
      expense('2', 'Accessory', 10),
      expense('3', 'Furniture', 10),
    ]
